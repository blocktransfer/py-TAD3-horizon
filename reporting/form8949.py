...
publicKey = BT_ISSUER #"GARLIC4DDPDXHAWNV5EBBKI7RSGGGDGEL5LH3F3N3U6I4G4WFYIN7GBG" # testing
  ...
  try:
    ...
    try:
      ...
      try:
        ...
        if(taker):
          for executions in op.tr.manage_sell_offer_result.success.offers_claimed:
            # print(executions.order_book.offer_id.int64) # 
            b = executions.order_book.offer_id.int64
            
            a = getOfferIDfromContraOfferID(b) # Check support for AMM fills
            
            print(f"{b} -> {a}")
            offerIDarr.append(a)
            return 1
        else:
          offerID = getOfferIDfromContraOfferID(op.tr.manage_sell_offer_result.success.offer.offer.offer_id.int64)
      except AttributeError:
        return 0
...
def getOfferIDfromContraOfferID(offerID):
