...
only reference origin info with mangageData, not washSale info

can keep local list of sellatloss

can set washmemo when buy wash 

 WALLET MEMO FOR TRADES: 
 type = open || exit
 originOfferID = {washSaleRefClosePT} || {offerID}
 f"{type}:{originOfferID}:{dissallowedWashSaleLoss}"
 ...
 OfferIDs will stay mapped to account with format offerID: {YEAR}:washSaleAdjustment until five years after the wash sale is closed (can be simple wallet wrap-up)
